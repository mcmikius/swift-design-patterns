...
let account = Account(accountName: "Ivan", balance: 1000)
let transactionManager = TransactionManager.shared
transactionManager.addTransaction(command: Deposit(account: account, amount: 200))
...
account.balance
transactionManager.addTransaction(command: Deposit(account: account, amount: 1000))
transactionManager.processingTransactions()
account.balance
